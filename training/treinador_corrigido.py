...
def main():
    ...
    RUN_NAME = "Alyssa-VITS-JA-EN-Final"
    ...
    alyssa_ja_dataset_config = BaseDatasetConfig(
        formatter="ljspeech",
        dataset_name="my_dataset_ja",
        meta_file_train="metadata_train_ja.csv",
        meta_file_val="metadata_val_ja.csv",
        path=os.path.join(CURRENT_PATH, "my_dataset"),
        language="ja",
    )

    alyssa_en_dataset_config = BaseDatasetConfig(
        formatter="ljspeech",
        dataset_name="my_dataset_en",
        meta_file_train="metadata_train_en.csv",
        meta_file_val="metadata_val_en.csv",
        path=os.path.join(CURRENT_PATH, "my_dataset"),
        language="en",
    )

    DATASETS_CONFIG_LIST = [alyssa_ja_dataset_config, alyssa_en_dataset_config]
    ...
    config = VitsConfig(
        ...
        run_name=RUN_NAME,
        project_name="Alyssa",
        run_description="Treinamento VITS final e otimizado para a IA Alyssa",
        ...
        plot_step=0,  # Desabilita a plotagem para evitar o erro no início do treino
        log_model_step=1000,
        save_step=5000,
        ...
        test_sentences=[
            ["これは日本語のテスト文です。", None, "ja"],
            ["This is a test sentence in English.", None, "en"],
        ],
