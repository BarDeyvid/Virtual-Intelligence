...
namespace internal {
    ...
    public:
        ...
        EmotionLLM(llama_model* shared_model, const llama_vocab* shared_vocab, const std::string& expert_id)
            : model(shared_model), vocab(shared_vocab), ctx(nullptr), smpl(nullptr) 
        {
            if (!model && !vocab) {
                throw std::runtime_error("EmotionLLM: Modelo ou Vocabular nulo recebido.");
            }
            ...
            if (!found) {
                throw std::runtime_error("EmotionLLM: Configuração do especialista com ID '" + expert_id + "' não encontrada no JSON.");
            }
            ...
            if (this->config.usa_LoRA && !this->config.lora_path.empty()) {
                ...
                if (!lora) {
                    throw std::runtime_error("EmotionLLM: Falha ao inicializar LoRA para: " + this->config.lora_path);
                }
                ...
                if (err != 0) {
                    llama_free(ctx); 
                    throw std::runtime_error("EmotionLLM: Falha ao aplicar LoRA: " + this->config.lora_path);
                }
        ...
        ~EmotionLLM() {
            ...
            std::cout << "Recursos [EmotionLLM] liberados." << std::endl;
        }
