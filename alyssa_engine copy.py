...
class AlyssaEngine:
    ...
    async def start_consumers(self) -> None:
        """
        Call this after the asyncio event‑loop has started.
        It spawns the background coroutine that processes the input queue.
        """
        # Only one consumer is needed – create it lazily
        if not hasattr(self, "_input_consumer_task"):
            # O loop está DEFINIDO graças à correção em main_test_fairy.py
            self._input_consumer_task = asyncio.create_task(
                self._consume_input_queue()
            )
...
if __name__ == "__main__":
    ...
    engine = AlyssaEngine(character_config=config)
    while True:
        user_text = input("Você: ")
        if user_text.lower() in ["sair", "exit", "quit"]:
            break
        response = asyncio.run(engine.handle_input(user_text))
        print(f"Alyssa: {response['alyssa_voice']}")